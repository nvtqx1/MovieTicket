        ...
        <div className="bg-[#18181b] p-6 rounded-xl flex flex-col h-full">
            ...
            <div className="space-y-3 flex-1 overflow-auto pr-2 custom-scrollbar">
                {data.map((m, index) => {
                    ...
                    return (
                        <div key={m.movieId ?? m.movieTitle} className="p-3 rounded-xl flex items-center justify-between gap-4 hover:bg-white/5 transition-colors border border-transparent hover:border-white/5">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <h3 className="font-bold text-sm line-clamp-1" title={m.movieTitle}>{m.movieTitle}</h3>
                                    <p className="text-[11px] text-gray-500 mt-1">
                                        {toNumber(m.showtimeCount).toLocaleString("vi-VN")} suất chiếu • {toNumber(m.ticketsSold).toLocaleString("vi-VN")} vé
                                    </p>
                                </div>
                            </div>
                            <span className="font-black text-green-400 text-sm whitespace-nowrap">
